 ...
 * HFR Memory Debugger with Unix Socket - Zero header dependencies
 * All socket structures and functions manually defined & resolved via kallsyms
 */
...
KPM_LICENSE("GPL v2");
KPM_AUTHOR("Surajit");
KPM_DESCRIPTION("Kernel memory r/w via Unix socket (zero deps)");
...
/* Manual socket structures - only what we need */
struct sockaddr_un {
...
typedef int (*kernel_bind_t)(struct socket *, struct sockaddr_un *, int);
...
/* Manual byte copy - no memcpy dependency */
...
static int start_socket_server(void)
{
    ...
    kpm_info("Unix socket listening: %s\n", HFR_SOCKET_PATH);
    return 0;
}
...
static long hfr_memory_init(const char *args, const char *event, void __user *reserved)
{
    ...
    if (start_socket_server() < 0) {
        kpm_err("Socket server startup failed\n");
        return -EFAULT;
    }

    server_running = 1;
    kpm_info("Module loaded - Unix socket at %s\n", HFR_SOCKET_PATH);
    return 0;
}
